...
KUBECTL_CLOUD_INSTALL_FILE=kube_cloud_install-${SERVERNAME}.sh
LOCALSERVERDIR=$(pwd)/${SERVERNAME}
KUBECONFIG_FILE=kube.${SERVERNAME}.config
TOKEN_FILE=token.${SERVERNAME}
...
docker run -d --rm -p ${HOSTPORT}:${HOSTPORT} \\
      ...
      --disable-agent 
if [ ! -e ${KUBECONFIG_FILE}  -o ! -e ${TOKEN_FILE} ]
then
	sleep 30
	echo "Copying the secrets, token to ${TOKEN_FILE} and kube.confg to ${KUBECONFIG_FILE}"
	docker cp k3s_${SERVERNAME}:/etc/rancher/k3s/k3s.yaml ${KUBECONFIG_FILE}
	sed -i -e "s/\(https:\/\/\)127.0.0.1/\1${HOSTIP}/" ${KUBECONFIG_FILE}
	docker cp k3s_${SERVERNAME}://var/lib/rancher/k3s/server/token ${TOKEN_FILE}
	if [ ! -e ${KUBECONFIG_FILE}  -o ! -e ${TOKEN_FILE} ]
	then
...
if [ ! -e "${KUBECTL_CLOUD_INSTALL_FILE}" ]
then
	cat<<EOF > "${KUBECTL_CLOUD_INSTALL_FILE}"
